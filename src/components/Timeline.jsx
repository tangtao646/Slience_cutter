    ...
    useEffect(() => {
        ...
        const onWheel = (e) => {
            if (e.ctrlKey || e.metaKey) {
                ...
                const totalX = scrollL + mouseX; // 鼠标指向的虚拟总像素位置
                const pointerTime = totalX / zoom; // 鼠标指向的时间点

                // 更细腻的阻尼系数: 1.05 而不是 1.1 或者更小 1.02
                // e.deltaY 本身在触控板上可能是很小的值，需要做归一化或者直接按符号判断
                // 为了平滑，我们使用较小的每步系数
                const dampingFit = 1.03;
                // 如果是触控板（e.deltaY通常比较小）
                // 简单判定：e.deltaY < 0 放大， > 0 缩小
                const newZoom = e.deltaY < 0 ? zoom * dampingFit : zoom / dampingFit;

                // 限制缩放范围
                const clampedZoom = Math.min(MAX_ZOOM, Math.max(minZoom, newZoom));
